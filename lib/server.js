...
export function requireDashboardAuthorized(req, res, environment = process.env) {
  if (environment.VERCEL_PRIVATE_SITE === 'true') return true;
  if (!environment.DASHBOARD_TOKEN) {
    res.status(503).json({ ok: false, error: 'Cloud CRM login is not configured' });
    return false;
  }
  if (hasBearerToken(req, environment.DASHBOARD_TOKEN)) return true;
  res.status(401).json({ ok: false, error: 'Cloud CRM login required' });
  return false;
}
