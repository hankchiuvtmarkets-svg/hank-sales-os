function databaseErrorMessage(error) {
  ...
  return [error?.message, error?.details, error?.hint, error?.code]
    .filter(Boolean)
    .join(' | ') || 'unknown Supabase/PostgREST error';
}

function dataOrThrow(result, context) {
  if (result.error) {
    const error = new Error(`${context}: ${databaseErrorMessage(result.error)}`);
...
export function createSupabaseRepository(db) {
  return {
    ...
    async countAgentSettings() {
      ...
      if (result.error) throw new Error(`read agent_settings: ${databaseErrorMessage(result.error)}`);
    ...
    async setLeadWorkflow({ lead, followed }) {
      const existingResult = await db
        .from('leads')
        .select('id,stage')
        .eq('profile_url', lead.profile_url)
        .maybeSingle();
      const existing = dataOrThrow(existingResult, 'read lead workflow');
      ...
      const stage = followed
        ...
        : existing?.stage === '已聯絡'
          ? '待審核'
          : existing?.stage || '待審核';
      const values = { ...lead, stage, updated_at: timestamp };
      const savedResult = existing
        ? await db.from('leads').update(values).eq('id', existing.id).select('*').single()
        : await db.from('leads').insert(values).select('*').single();
      const saved = dataOrThrow(savedResult, 'save lead workflow');
      if (followed) {
        const activityResult = await db.from('activities').insert({
          lead_id: saved.id,
          type: 'contact_marked',
          details: { source: 'dashboard' },
          completed_at: timestamp
        }).select('id').single();
        dataOrThrow(activityResult, 'record lead follow-up');
      }
      return saved;
    },
