...
Browser Dashboard
  ...
  -> Supabase 雲端結果（唯讀顯示）
  -> /data/auto-leads.json（每日 GitHub 自動名單，不需 Dashboard Token）
```

### 每日零金鑰名單

`data/auto-leads.json` 是可部署的公開研究摘要。ChatGPT 排程每天決定韓國／日本目標類型、產生搜尋策略、搜尋公開頁面、去除文章與重複帳號、分析痛點與合作可能性，再更新這個檔案。GitHub 更新會觸發 Vercel 自動部署，所以首頁不登入也能看到最新目標。
