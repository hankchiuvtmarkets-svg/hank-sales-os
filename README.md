...
- Vercel Authentication 保護整個私人工作台；登入 Vercel 後不再要求額外 token／解鎖碼
...
Browser Dashboard
  ...
  -> Vercel Authentication：只允許已授權的 Vercel 帳號
  -> /api/agent-status + /api/cloud-leads
  -> Supabase 雲端結果（唯讀顯示）
  -> /api/automatic-feed（server-side 解密後回傳）
  -> main/data/auto-leads.json（GitHub 只保存密文，私人 Preview 即時讀取最新版）
```

### 每日零金鑰名單

`data/auto-leads.json` 是 AES-256-GCM 加密後的研究資料。安全排程輪替日本、韓國、香港、澳門的目標類型，產生當地語言搜尋策略、搜尋公開頁面、去除文章與重複帳號、分析痛點與合作可能性，再加密更新這個檔案。加密內容另外保留最多 2,000 個已辨識對象與最近 90 次搜尋紀錄；即使某位對象沒有進入目前的高分名單，之後再次出現仍會被辨識為舊對象。首頁可用市場按鈕分流顯示。Vercel Authentication 先在平台邊緣驗證帳號，再由 serverless API 使用環境變數解密；解鎖密語不會進入瀏覽器，公開 repo 也不會暴露目標帳號、評分、痛點或破冰草稿。
...
- `lib/automatic-feed.js`：server-side 加密名單解密與格式驗證
...
| `/api/manual-run` | POST | Vercel Authentication | 「開始持續搜尋」的即時執行入口；缺 provider keys 時切換安全排程備援 |
| `/api/cron-discover` | GET/POST | `Bearer CRON_SECRET` | Vercel Cron 入口 |
| `/api/agent-status` | GET | Vercel Authentication | 執行狀態、限制與待批准草稿 |
| `/api/cloud-leads` | GET | Vercel Authentication | 讀取雲端 leads |
| `/api/pending-drafts` | GET | Vercel Authentication | 讀取待批准草稿 |
| `/api/automatic-feed` | GET | Vercel Authentication | server-side 解密並讀取自動名單；禁止快取 |
...
| `VERCEL_PRIVATE_SITE` | 私人 Production 設為 `true`；啟用 Vercel Authentication 後才可設定 |
| `AUTO_FEED_PASSPHRASE` | 自動名單 server-side 解密密語；只能存在 Vercel Environment Variables |
| `DASHBOARD_TOKEN` | 舊版／本機開發的備援驗證；私人 Production 使用 Vercel Authentication 時可保留但前端不會讀取 |
...
2. 在 Vercel Deployment Protection 啟用免費的 Standard Protection。
3. 使用 `private-dashboard` Preview branch 作為私人工作台；正式網域不提供客戶資料。
4. 在 Vercel Project Settings → Environment Variables 為 Preview 新增缺少的變數；確認保護已生效後才設定 `VERCEL_PRIVATE_SITE=true`。
5. 重新部署 Preview，讓新增／更新的環境變數生效。
6. 開啟私人 Preview 網址並以有專案權限的 Vercel 帳號登入，確認名單直接顯示且不再出現自製密碼頁。
7. 開啟 `/api/health`，確認 `ok: true` 且 `agentSettingsRows` 至少為 `1`。
